	...
	TimeRemaining(10)
{
	...
	GameStateClass = ANetPlayerState::StaticClass();
}
...
void ANetGameMode::RestartTimeFunction()
{
	TimeRemaining = GameLenght;
	TimeFunction();
}

void ANetGameMode::TimeFunction()
{
	TimeRemaining--;

	if (TimeRemaining <= 0)
	{
		GEngine->AddOnScreenDebugMessage(-1, 2.5f, FColor::Red, TEXT("TIME OVER"));
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, .9f, FColor::Red, FString::Printf(TEXT("%i"), TimeRemaining));
		GWorld->GetTimerManager().SetTimer(TimeTimer, this, &ANetGameMode::TimeFunction, 1.0f, false);
	}
}
